...
SOURCE_ORDERS = {"dataset": DATASET_SOURCES, "model": MODEL_SOURCES}

# TODO: add missing ones
DATASET_TARGETS = ["dermamnist"]


def parse_args():
    parser = argparse.ArgumentParser(
        description="Compute transferability scores between source and target"
    )
    parser.add_argument(
        "--source_type",
        type=str,
        choices=["dataset", "model"],
        help="either 'dataset' (dataset transfer) or 'model' (architecture transfer)",
        required=True,
    )
    parser.add_argument(
        "--runs",
    ...
    #     "--source_path",
    #     type=str,
    #     help="path to source dataset(s) or model",
    #     required=True,
    # )
    # parser.add_argument(
    #     "--target_path",
    ...
    parser.add_argument(
        "--method",
        type=str,
        help="choose a tranferability method (logme, leep, nleep, sfda, parc, ncti, lp, fu)",
        required=True,
    )
...
class MyDataset(Dataset):
    def __init__(
        self, data, targets, transform=None, target_transform=None, as_rgb=False
    ):
        self.data = data
...
def load_data(source_flag, target_flag, run, split):
    ...
    target_data = np.load(
        # "../data/medmnist_data_subsets/" + target_flag + "_fine-tune_224.npz"
        "../splits/dermamnist_splits.npz"
    )
    trainset = MyDataset(
        target_data["train_images"][
            target_data[f"train_idx_run{run}_split-{split}pct"]
        ],
...
def get_fc(source_flag, target_flag, run, split):

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    train_set = load_data(source_flag, target_flag, run, split)
    data_loader = DataLoader(dataset=train_set, batch_size=32, shuffle=False)
    ...
    if source_flag == "densenet":
        ...
        if source_flag == "imagenet":
            ...
            info = INFO[target_flag]
            if (
                target_flag == "pneumoniamnist"
                or target_flag == "organamnist"
                or target_flag == "organcmnist"
                or target_flag == "organsmnist"
            ):
                source_classes = 69
            ...
            # model_path = "../weights_dermamnist/resnet18_224_1.pth"
            model_path = "../models/" + source_flag + "_resnet18_224_1.pth"
            net = torchvision.models.resnet18(num_classes=source_classes)
            net.load_state_dict(
                torch.load(model_path, map_location=device)["net"], strict=False
            )
        last = net.fc
...
def get_layers(source_flag, target_flag, run, split):
    ...
    if source_flag == "densenet":
        ...
        second = net.features[1][0].block[0][
            0
        ...
        second = net.stage2[0].branch1[
            0
        ]  # Conv2d(24, 24, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), groups=24, bias=False)
    elif source_flag == "resnet":
        net = torchvision.models.resnet18(weights="IMAGENET1K_V1")
        net.fc = nn.Identity()
        first = (
    ...
    data_loader = DataLoader(
        ...
        batch_size=math.floor(
            len(train_set) / 6
        ),  # if target_flag in ['pneumoniamnist', 'breastmnist'] else 256,
        shuffle=False,
    )
...
def compute_transferability_score(target_order, source_order, method, run, split):
    ...
    for target in target_order:  # TODO: adjust this line to run for several targets
        seli_scores = []
        ncc_scores = []
        vc_scores = []
        scores = []
        for source in source_order:
            if method == "fu":
                model, first, second, data_loader, device = get_layers(
                    source, target, run, split
                )
                score = FU_Score(model, first, second, data_loader, device)
                scores.append(score)
            else:
                X, output, y = get_fc(source, target, run, split)
                if method == "logme":
        ...
        if method == "ncti":
            seli_df[target] = (seli_scores - np.nanmin(seli_scores)) / (
                np.nanmax(seli_scores) - np.nanmin(seli_scores)
            )
            ncc_df[target] = (ncc_scores - np.nanmin(ncc_scores)) / (
                np.nanmax(ncc_scores) - np.nanmin(ncc_scores)
            )
            vc_df[target] = (vc_scores - np.nanmin(vc_scores)) / (
                np.nanmax(vc_scores) - np.nanmin(vc_scores)
            )
...
def main():
    ...
    source_order = SOURCE_ORDERS[args.source_type]
    target_order = DATASET_TARGETS

    for run in range(1, args.runs + 1):
        print(f"Run {run}/{args.runs}")
        for split in args.splits:
            print(f"Split {split}")
            score_df = compute_transferability_score(
                target_order, source_order, args.method, run, split
            )
            score_df.to_csv(
                f"./results/{args.method}_{args.source_type}_run{run}_{split}pct.csv"
            )
